...
export default function SettingsScreen() {
  ...
  async function exportCSV() {
    ...
    const rows = collection.map(item => {
      ...
      const name = (product?.name ?? item.productId).replace(/,/g, ' ');
      const notes = (item.notes ?? '').replace(/,/g, ' ');
      return `${item.productId},${name},${item.quantity},${item.purchasePrice},${item.purchaseDate},${item.condition ?? 'NM'},${notes}`;
    });
    const csv = [CSV_HEADER, ...rows].join('\n');
    await Share.share({ message: csv, title: 'VaultMark Collection' });
  }
